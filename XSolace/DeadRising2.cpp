...
namespace DeadRising2 {
	...
	namespace gameFuncs {
		...
		void(__fastcall *AddMoney)(uint32_t result, int money, int a3, int a4) = reinterpret_cast<void(__fastcall*)(uint32_t  result, int money, int a1, int a2)>(0x82601038);
	}
	...
	namespace funcs {
		...
		bool GodMode() {
			if (!vars::god) {
				xTools::WriteUInt32(vars::stHealth, vars::nop);
				xTools::Xam::XNotify("God Mode : Enabled");
				xTools::Xam::PulseController();
				vars::god = true;
			}
			else {
				xTools::WriteUInt32(vars::stHealth, vars::godMode_off);
		...
		void ModPP() {
			uint32_t currPP = xTools::ReadUInt32(vars::currentPP);
			xTools::WriteUInt32(vars::currentPP, 999999);
		...
		bool ShowGraphs() {
			if (!vars::graphs) {
				xTools::WriteUInt8(vars::enableRenderPerformanceInfoChartz, 1);
				xTools::WriteUInt8(vars::EnableRenderCPUBudgetChartz, 1);
				xTools::Xam::XNotify("Performance Graphs : Enabled");
				xTools::Xam::PulseController();
				vars::graphs = true;
			}
			else {
				xTools::WriteUInt8(vars::enableRenderPerformanceInfoChartz, 0);
				xTools::WriteUInt8(vars::EnableRenderCPUBudgetChartz, 0);
		...
		void _DeadRising2::WelcomePage() {
			...
			const wchar_t* buttonLabels[] = { L"Next", L"Reboot", L"Change Power LED"};
			uint32_t buttonPressedIndex = 0;
			uint32_t result = xTools::Xam::ShowMessageBox(
				L"Dead Rising 2 Test",
				L"Page 1 Test",
